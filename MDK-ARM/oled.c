 ...
 * @param color 颜色
 */
void OLED_snake_food(uint8_t x ,uint8_t y ,OLED_ColorMode color)
{
  OLED_SetPixel( x,  y,  color);
  OLED_SetPixel( x+1,  y+1,  color);
  OLED_SetPixel( x+1,  y-1,  color);
  OLED_SetPixel( x-1,  y+1,  color);
  OLED_SetPixel( x-1,  y-1,  color);
  OLED_SetPixel( x+1,  y,  color);
  OLED_SetPixel( x-1,  y,  color);
  OLED_SetPixel( x,  y+1,  color);
  OLED_SetPixel( x,  y-1,  color);
}
...
* @brief 绘制一个随机食物
 */
void OLED_snake_foodx(void)
{
  uint8_t x ,y ;
  x=(rand()%40)*3+5;
  y=(rand()%16)*3+13;
  OLED_snake_food( x , y ,OLED_COLOR_NORMAL);
}
 ...
 * @param color 颜色
 */
void OLED_snake_body(uint8_t x ,uint8_t y ,OLED_ColorMode color)
{
  OLED_SetPixel( x,  y,  color);
  OLED_SetPixel( x+1,  y-1,  color);
  OLED_SetPixel( x-1,  y-1,  color);
